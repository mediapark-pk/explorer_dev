import React from 'react';
import { useStyles } from 'src/pages/Forget/component/TextFieldOutlinedActive5/style';
import Typography from '@material-ui/core/Typography';
import FormShape5 from 'src/pages/Forget/component/FormShape5';

interface ITextFieldOutlinedActive5Props {
}

const TextFieldOutlinedActive5: React.FC<ITextFieldOutlinedActive5Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <div className={classes.div} >
                <Typography className={classes.typography} >
                    Your email
                </Typography>
            </div>
            <div className={classes.div1} >
                <div className={classes.textCursor} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
                <Typography className={classes.typography1} >
                    Input text
                </Typography>
            </div>
            <FormShape5 />
        </div>
    );
};

export default TextFieldOutlinedActive5;



