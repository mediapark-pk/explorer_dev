import React from 'react';
import { useStyles } from 'src/pages/delegates/component/RowsPerPage/style';
import Typography from '@material-ui/core/Typography';
import DropDown from 'src/pages/delegates/component/DropDown';

interface IRowsPerPageProps {
}

const RowsPerPage: React.FC<IRowsPerPageProps> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Rows per page:
            </Typography>
            <DropDown />
            <Typography className={classes.typography1} >
                10 of 500 
            </Typography>
            <div
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
            <div
                className={classes.button1}
                classes={{ root: classes.root2 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default RowsPerPage;



