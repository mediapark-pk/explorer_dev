...
export default class DelegatesService {
    ...
    @action
    async getDelegates(request: Request): Promise<IListResponse<Delegate>> {
    ...
    async getDelegatesInfo(): Promise<Response<RawDelegatesInfo>> {
        return this.socket.query(SocketCode.GET_DELEGATES_INFO);
    }

    onDelegatesInfoUpdate(): Observable<Response<RawDelegatesInfo>> {
        return this.socket.getEvent(SocketCode.ON_DELEGATES_INFO_UPDATE);
    }
}
