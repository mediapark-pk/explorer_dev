import React from 'react';
import { useStyles } from 'src/pages/Login/component/TextFieldOutlinedActive/style';
import Typography from '@material-ui/core/Typography';
import FormShape from 'src/pages/Login/component/FormShape';

interface ITextFieldOutlinedActiveProps {
}

const TextFieldOutlinedActive: React.FC<ITextFieldOutlinedActiveProps> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <div className={classes.div} >
                <Typography className={classes.typography} >
                    Email
                </Typography>
            </div>
            <div className={classes.div1} >
                <div className={classes.textCursor} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
                <Typography className={classes.typography1} >
                    Input text
                </Typography>
            </div>
            <FormShape />
        </div>
    );
};

export default TextFieldOutlinedActive;



