import React from 'react';
import { useStyles } from 'src/pages/Login/component/BaseSurfaceShapeSurfaceMasterShape/style';

interface IBaseSurfaceShapeSurfaceMasterShapeProps {
}

const BaseSurfaceShapeSurfaceMasterShape: React.FC<IBaseSurfaceShapeSurfaceMasterShapeProps> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <div className={classes.baseSurfaceShapeAtomsSquare} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default BaseSurfaceShapeSurfaceMasterShape;



