import React from 'react';
import { useStyles } from 'src/pages/block/component/ButtonAddToFav4/style';
import Typography from '@material-ui/core/Typography';

interface IButtonAddToFav4Props {
}

const ButtonAddToFav4: React.FC<IButtonAddToFav4Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Add to favorites
            </Typography>
            <div className={classes.custom1582297} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default ButtonAddToFav4;



