import React from 'react';
import { useStyles } from 'src/pages/delegate/component/ButtonAddToFav1/style';
import Typography from '@material-ui/core/Typography';

interface IButtonAddToFav1Props {
}

const ButtonAddToFav1: React.FC<IButtonAddToFav1Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Add to favorites
            </Typography>
            <div className={classes.baselineFavoriteBorder24px} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default ButtonAddToFav1;



