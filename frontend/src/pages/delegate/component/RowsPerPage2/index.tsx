import React from 'react';
import { useStyles } from 'src/pages/delegate/component/RowsPerPage2/style';
import Typography from '@material-ui/core/Typography';
import DropDown1 from 'src/pages/delegate/component/DropDown1';

interface IRowsPerPage2Props {
}

const RowsPerPage2: React.FC<IRowsPerPage2Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Rows per page:
            </Typography>
            <DropDown1 />
            <Typography className={classes.typography1} >
                10 of 500 
            </Typography>
            <div
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
            <div
                className={classes.button1}
                classes={{ root: classes.root2 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default RowsPerPage2;



