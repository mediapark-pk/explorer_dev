import React from 'react';
import { useStyles } from 'src/pages/delegate/component/ButtonAddToFav/style';
import Typography from '@material-ui/core/Typography';

interface IButtonAddToFavProps {
}

const ButtonAddToFav: React.FC<IButtonAddToFavProps> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Add to favorites
            </Typography>
            <div className={classes.baselineFavoriteBorder24px} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default ButtonAddToFav;



