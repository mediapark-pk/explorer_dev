import React from 'react';
import { useStyles } from 'src/pages/addresses/component/RowsPerPage8/style';
import Typography from '@material-ui/core/Typography';
import DropDown4 from 'src/pages/addresses/component/DropDown4';

interface IRowsPerPage8Props {
}

const RowsPerPage8: React.FC<IRowsPerPage8Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Rows per page:
            </Typography>
            <DropDown4 />
            <Typography className={classes.typography1} >
                10 of 500 
            </Typography>
            <div
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
            <div
                className={classes.button1}
                classes={{ root: classes.root2 }}
            >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default RowsPerPage8;



