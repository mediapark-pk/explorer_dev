import React from 'react';
import { useStyles } from 'src/pages/addresses/component/DelegatesBlock5/style';
import Typography from '@material-ui/core/Typography';

interface IDelegatesBlock5Props {
}

const DelegatesBlock5: React.FC<IDelegatesBlock5Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <div className={classes.custom25123066} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
            <Typography className={classes.typography} >
                Addresses
            </Typography>
            <Typography className={classes.typography1} >
                (1)
            </Typography>
        </div>
    );
};

export default DelegatesBlock5;



