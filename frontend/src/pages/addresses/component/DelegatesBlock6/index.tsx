import React from 'react';
import { useStyles } from 'src/pages/addresses/component/DelegatesBlock6/style';
import Typography from '@material-ui/core/Typography';

interface IDelegatesBlock6Props {
}

const DelegatesBlock6: React.FC<IDelegatesBlock6Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <div className={classes.custom25123034} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
            <Typography className={classes.typography} >
                Transactions
            </Typography>
            <Typography className={classes.typography1} >
                (6)
            </Typography>
        </div>
    );
};

export default DelegatesBlock6;



