import React from 'react';
import { useStyles } from 'src/pages/addresses/component/BackButton3/style';
import Button from '@material-ui/core/Button';
import Typography from '@material-ui/core/Typography';

interface IBackButton3Props {
}

const BackButton3: React.FC<IBackButton3Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Button
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <div className={classes.custom2518519} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
                <Typography className={classes.typography} >
                    Back
                </Typography>
            </Button>
        </div>
    );
};

export default BackButton3;



