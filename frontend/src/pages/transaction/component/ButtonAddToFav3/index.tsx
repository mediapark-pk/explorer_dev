import React from 'react';
import { useStyles } from 'src/pages/transaction/component/ButtonAddToFav3/style';
import Typography from '@material-ui/core/Typography';

interface IButtonAddToFav3Props {
}

const ButtonAddToFav3: React.FC<IButtonAddToFav3Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Add to favorites
            </Typography>
            <div className={classes.custom1831086} >
                <img src='data:image/svg+xml;base64,'/>
            </div>
        </div>
    );
};

export default ButtonAddToFav3;



