import React from 'react';
import { useStyles } from 'src/pages/transaction/component/RowsPerPage12/style';
import Typography from '@material-ui/core/Typography';
import DropDown6 from 'src/pages/transaction/component/DropDown6';
import Button from '@material-ui/core/Button';

interface IRowsPerPage12Props {
}

const RowsPerPage12: React.FC<IRowsPerPage12Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Rows per page:
            </Typography>
            <DropDown6 />
            <Typography className={classes.typography1} >
                10 of 500 
            </Typography>
            <Button
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <div className={classes.frame} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
            </Button>
            <Button
                className={classes.button1}
                classes={{ root: classes.root2 }}
            >
                <div className={classes.frame1} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
            </Button>
        </div>
    );
};

export default RowsPerPage12;



