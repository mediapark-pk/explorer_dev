import React from 'react';
import { useStyles } from 'src/pages/transactions/component/RowsPerPage14/style';
import Typography from '@material-ui/core/Typography';
import DropDown7 from 'src/pages/transactions/component/DropDown7';
import Button from '@material-ui/core/Button';

interface IRowsPerPage14Props {
}

const RowsPerPage14: React.FC<IRowsPerPage14Props> = ({ }) => {
    const classes = useStyles({});

    return (
        <div className={classes.root} >
            <Typography className={classes.typography} >
                Rows per page:
            </Typography>
            <DropDown7 />
            <Typography className={classes.typography1} >
                10 of 500 
            </Typography>
            <Button
                className={classes.button}
                classes={{ root: classes.root1 }}
            >
                <div className={classes.frame} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
            </Button>
            <Button
                className={classes.button1}
                classes={{ root: classes.root2 }}
            >
                <div className={classes.frame1} >
                    <img src='data:image/svg+xml;base64,'/>
                </div>
            </Button>
        </div>
    );
};

export default RowsPerPage14;



